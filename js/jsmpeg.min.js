...
if (document.readyState === "complete") { JSMpeg.CreateVideoElements() } else { document.addEventListener("DOMContentLoaded", JSMpeg.CreateVideoElements) } JSMpeg.VideoElement = function() {
    "use strict";
    var VideoElement = function(element) {
        ...
        for (var option in element.dataset) { try { options[option] = JSON.parse(element.dataset[option]) } catch (err) { options[option] = element.dataset[option] } } this.player = new JSMpeg.Player(url, options);
    ...
    VideoElement.prototype.onUnlockAudio = function(element, ev) {
        ...
        this.player.audioOut.unlock(function() {
            if (this.unmuteButton) { this.unmuteButton.style.display = "none" } element.removeEventListener("touchstart", this.unlockAudioBound);
...
JSMpeg.Player = function() {
    ...
    Player.prototype.play = function(ev) {
        console.log(this.options.audioSrc);
        if (this.animationId) { return } this.animationId = requestAnimationFrame(this.update.bind(this));
        this.wantsToPlay = true;
        this.paused = false
        console.log("beginplay")
    };
    Player.prototype.pause = function(ev) {
        if (this.paused) { return } cancelAnimationFrame(this.animationId);
    ...
    Player.prototype.updateForStaticFile = function() {
        ...
        if (this.audio && this.audio.canPlay) { while (!notEnoughData && this.audio.decodedTime - this.audio.currentTime < .25) { notEnoughData = !this.audio.decode() } if (this.video && this.video.currentTime < this.audio.currentTime) { notEnoughData = !this.video.decode() } headroom = this.demuxer.currentTime - this.audio.currentTime } else if (this.video) {
            ...
            if (this.video && lateTime > 0) { if (lateTime > frameTime * 2) { this.startTime += lateTime } notEnoughData = !this.video.decode() } headroom = this.demuxer.currentTime - targetTime
        }
...
JSMpeg.BitBuffer = function() {
    ...
    BitBuffer.prototype.evict = function(sizeNeeded) {
        ...
        if (this.bytes.copyWithin) { this.bytes.copyWithin(0, bytePos, this.byteLength) } else { this.bytes.set(this.bytes.subarray(bytePos, this.byteLength)) } this.byteLength = this.byteLength - bytePos;
...
JSMpeg.Source.AjaxProgressive = function() {
    ...
    AjaxProgressiveSource.prototype.loadNextChunk = function() {
        ...
        if (start >= this.fileSize || this.aborted) { this.completed = true; if (this.onCompletedCallback) { this.onCompletedCallback(this) } return } this.isLoading = true;
        ...
        if (start === 0) { this.request.onprogress = this.onProgress.bind(this) } this.request.open("GET", this.url + "?" + start + "-" + end);
    ...
    AjaxProgressiveSource.prototype.onChunkLoad = function(data) {
        ...
        if (this.destination) { this.destination.write(data) } this.loadTime = JSMpeg.Now() - this.loadStartTime;
...
JSMpeg.Demuxer.TS = function() {
    ...
    TS.prototype.resync = function() {
        ...
        for (var i = 0; i < 187; i++) { if (this.bits.bytes[byteIndex + i] === 71) { var foundSync = true; for (var j = 1; j < 5; j++) { if (this.bits.bytes[byteIndex + i + 188 * j] !== 71) { foundSync = false; break } } if (foundSync) { this.bits.index = byteIndex + i + 1 << 3; return true } } } console.warn("JSMpeg: Possible garbage data. Skipping.");
...
JSMpeg.Decoder.Base = function() {
    ...
    BaseDecoder.prototype.seek = function(time) {
        if (!this.collectTimestamps) { return } this.timestampIndex = 0;
        for (var i = 0; i < this.timestamps.length; i++) { if (this.timestamps[i].time > time) { break } this.timestampIndex = i }
    ...
    BaseDecoder.prototype.advanceDecodedTime = function(seconds) {
        if (this.collectTimestamps) {
            ...
            for (var i = this.timestampIndex; i < this.timestamps.length; i++) { if (this.timestamps[i].index > currentIndex) { break } newTimestampIndex = i }
...
JSMpeg.Decoder.MPEG1Video = function() {
    ...
    MPEG1.prototype.decode = function() {
        ...
        if (this.bits.findStartCode(MPEG1.START.PICTURE) === -1) { var bufferedBytes = this.bits.byteLength - (this.bits.index >> 3); return false } this.decodePicture();
    ...
    MPEG1.prototype.decodeSequenceHeader = function() {
        ...
        if (this.bits.read(1)) { for (var i = 0; i < 64; i++) { this.customIntraQuantMatrix[MPEG1.ZIG_ZAG[i]] = this.bits.read(8) } this.intraQuantMatrix = this.customIntraQuantMatrix }
        if (this.bits.read(1)) {
            for (var i = 0; i < 64; i++) {
    ...
    MPEG1.prototype.decodePicture = function(skipOutput) {
        ...
        if (this.pictureType === MPEG1.PICTURE_TYPE.PREDICTIVE) {
            ...
            if (this.forwardFCode === 0) { return } this.forwardRSize = this.forwardFCode - 1;
    ...
    MPEG1.prototype.decodeSlice = function(slice) {
        ...
        while (this.bits.read(1)) { this.bits.skip(8) } do { this.decodeMacroblock() } while (!this.bits.nextBytesAreStartCode())
    };
    ...
    MPEG1.prototype.decodeMacroblock = function() {
        ...
        for (var block = 0, mask = 32; block < 6; block++) { if ((cbp & mask) !== 0) { this.decodeBlock(block) } mask >>= 1 }
    };
    MPEG1.prototype.decodeMotionVectors = function() {
        ...
        if (this.macroblockMotFw) {
            ...
            if (code !== 0 && this.forwardF !== 1) {
                ...
                if (code < 0) { d = -d }
            } else { d = code } this.motionFwHPrev += d;
            if (this.motionFwHPrev > (this.forwardF << 4) - 1) { this.motionFwHPrev -= this.forwardF << 5 } else if (this.motionFwHPrev < -this.forwardF << 4) { this.motionFwHPrev += this.forwardF << 5 } this.motionFwH = this.motionFwHPrev;
            if (this.fullPelForward) { this.motionFwH <<= 1 } code = this.readHuffman(MPEG1.MOTION);
            if (code !== 0 && this.forwardF !== 1) {
                r = this.bits.read(this.forwardRSize);
                d = (Math.abs(code) - 1 << this.forwardRSize) + r + 1;
                if (code < 0) { d = -d }
            } else { d = code } this.motionFwVPrev += d;
            if (this.motionFwVPrev > (this.forwardF << 4) - 1) { this.motionFwVPrev -= this.forwardF << 5 } else if (this.motionFwVPrev < -this.forwardF << 4) { this.motionFwVPrev += this.forwardF << 5 } this.motionFwV = this.motionFwVPrev;
    ...
    MPEG1.prototype.decodeBlock = function(block) {
        ...
        if (this.macroblockIntra) {
            ...
            if (block < 4) { this.dcPredictorY = this.blockData[0] } else if (block === 4) { this.dcPredictorCr = this.blockData[0] } else { this.dcPredictorCb = this.blockData[0] } this.blockData[0] <<= 3 + 5;
        ...
        while (true) {
            ...
            n++;
            level <<= 1;
            if (!this.macroblockIntra) { level += level < 0 ? -1 : 1 } level = level * this.quantizerScale * quantMatrix[dezigZagged] >> 4;
            if ((level & 1) === 0) { level -= level > 0 ? 1 : -1 }
            if (level > 2047) { level = 2047 } else if (level < -2048) { level = -2048 } this.blockData[dezigZagged] = level * MPEG1.PREMULTIPLIER_MATRIX[dezigZagged]
        }
...
JSMpeg.Decoder.MPEG1VideoWASM = function() {
    ...
    MPEG1WASM.prototype.initializeWasmDecoder = function() {
        if (!this.module.instance) { console.warn("JSMpeg: WASM module not compiled yet"); return } this.instance = this.module.instance;
    ...
    MPEG1WASM.prototype.destroy = function() { if (!this.decoder) { return } this.functions._mpeg1_decoder_destroy(this.decoder) };
    ...
    MPEG1WASM.prototype.bufferSetIndex = function(index) { if (!this.decoder) { return } this.functions._mpeg1_decoder_set_index(this.decoder, index) };
    ...
    MPEG1WASM.prototype.write = function(pts, buffers) { JSMpeg.Decoder.Base.prototype.write.call(this, pts, buffers); if (!this.hasSequenceHeader && this.functions._mpeg1_decoder_has_sequence_header(this.decoder)) { this.loadSequnceHeader() } };
...
JSMpeg.Decoder.MP2Audio = function() {
    ...
    MP2.prototype.decode = function() {
        ...
        if (this.destination) { this.destination.play(this.sampleRate, this.left, this.right) } this.advanceDecodedTime(this.left.length / this.sampleRate);
        var elapsedTime = JSMpeg.Now() - startTime;
        if (this.onDecodeCallback) { this.onDecodeCallback(this, elapsedTime) }
        return true
    };
            ...
            for (var granule = 0; granule < 4; granule++) {
                ...
                for (var p = 0; p < 3; p++) {
                    ...
                    for (var ch = 0; ch < 2; ch++) {
                        ...
                        while (vIndex < 1024) {
                            for (var i = 0; i < 32; ++i) { this.U[i] += this.D[dIndex++] * this.V[vIndex++] } vIndex += 128 - 32;
                            dIndex += 64 - 32
                        }
...
JSMpeg.Decoder.MP2AudioWASM = function() {
    ...
    MP2WASM.prototype.destroy = function() { if (!this.decoder) { return } this.functions._mp2_decoder_destroy(this.decoder) };
    ...
    MP2WASM.prototype.bufferSetIndex = function(index) { if (!this.decoder) { return } this.functions._mp2_decoder_set_index(this.decoder, index) };
...
JSMpeg.Renderer.WebGL = function() {
    ...
    WebGLRenderer.prototype.render = function(y, cb, cr, isClampedArray) {
        ...
        var w = this.width + 15 >> 4 << 4,
            ...
            w2 = w >> 1,
            h2 = h >> 1;
        if (isClampedArray && this.shouldCreateUnclampedViews) { y = new Uint8Array(y.buffer), cb = new Uint8Array(cb.buffer), cr = new Uint8Array(cr.buffer) } gl.useProgram(this.program);
...
JSMpeg.AudioOutput.WebAudio = function() {
    ...
    WebAudioOut.prototype.play = function(sampleRate, left, right) {
        if (!this.enabled) { return }
        if (!this.unlocked) { var ts = JSMpeg.Now(); if (this.wallclockStartTime < ts) { this.wallclockStartTime = ts } this.wallclockStartTime += left.length / sampleRate; return } this.gain.gain.value = this.volume;
    ...
    WebAudioOut.prototype.unlock = function(callback) {
        if (this.unlocked) { if (callback) { callback() } return } this.unlockCallback = callback;
...
JSMpeg.WASMModule = function() {
    ...
    WASM.prototype.write = function(buffer) { this.loadFromBuffer(buffer, this.onInitCallback) };
    ...
    WASM.prototype.loadFromBuffer = function(buffer, callback) {
        ...
        if (!this.moduleInfo) { this.callback && this.callback(null); return } this.memory = new WebAssembly.Memory({ initial: 256 });
        ...
        WebAssembly.instantiate(buffer, { env: env }).then(function(results) {
            ...
            if (this.instance.exports.__post_instantiate) { this.instance.exports.__post_instantiate() } this.createHeapViews();
